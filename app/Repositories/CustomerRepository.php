...
abstract class CustomerRepository implements RepositoryInterface
{
    ...
    public function find($id)
    {
        try 
        {
            $customer = $this->model::with('market.area', 'special_discounts.product', 'ledgers.invoice.invoice_products', 'invoices.invoice_products', 'customer_images', 'ledgers.expense', 'expenses')->find($id);
            if(!$customer)
            {
                return [
                    'success' => false,
    ...
    public function search_customers_all($query)
    {
        // foreign fields
        // markets
        $markets = Market::select('id')->where('name', 'LIKE', '%'.$query.'%')->get();
    ...
    {
        return count($this->model->where('status', $status)->get());
    }
}
